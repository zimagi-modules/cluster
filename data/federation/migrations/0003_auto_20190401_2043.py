# Generated by Django 2.1.7 on 2019-04-02 00:43

from django.db import migrations, models
import django.db.models.deletion
import systems.models.fields


class Migration(migrations.Migration):

    dependencies = [
        ('federation', '0002_auto_20190313_2131'),
    ]

    operations = [
        migrations.AlterModelOptions(
            name='federation',
            options={'ordering': ['name'], 'verbose_name': 'federation', 'verbose_name_plural': 'federations'},
        ),
        migrations.RemoveField(
            model_name='federation',
            name='type',
        ),
        migrations.AddField(
            model_name='federation',
            name='provider_type',
            field=models.CharField(editable=False, max_length=128, null=True),
        ),
        migrations.AlterField(
            model_name='federation',
            name='config',
            field=systems.models.fields.EncryptedDataField(default={}, editable=False),
        ),
        migrations.AlterField(
            model_name='federation',
            name='created',
            field=models.DateTimeField(editable=False, null=True),
        ),
        migrations.AlterField(
            model_name='federation',
            name='environment',
            field=models.ForeignKey(editable=False, null=True, on_delete=django.db.models.deletion.PROTECT, related_name='federation_relation', to='environment.Environment'),
        ),
        migrations.AlterField(
            model_name='federation',
            name='id',
            field=models.CharField(editable=False, max_length=64, primary_key=True, serialize=False),
        ),
        migrations.AlterField(
            model_name='federation',
            name='name',
            field=models.CharField(editable=False, max_length=256),
        ),
        migrations.AlterField(
            model_name='federation',
            name='networks',
            field=models.ManyToManyField(editable=False, related_name='federation_relation', to='network.Network'),
        ),
        migrations.AlterField(
            model_name='federation',
            name='state_config',
            field=systems.models.fields.EncryptedDataField(default={}, editable=False),
        ),
        migrations.AlterField(
            model_name='federation',
            name='updated',
            field=models.DateTimeField(editable=False, null=True),
        ),
        migrations.AlterField(
            model_name='federation',
            name='variables',
            field=systems.models.fields.EncryptedDataField(default={}, editable=False),
        ),
    ]
